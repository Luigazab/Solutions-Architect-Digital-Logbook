            ...
            <div className="bg-white p-4 rounded-xl shadow">
                <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center mb-4">
                    <div>
                        ...
                        <Subtitle>Click row to view/edit customer details</Subtitle>
                    </div>
                    ...
                    <div className="flex gap-2">
                        <button type="button" onClick={handleAddCustomer} className="inline-flex items-center border rounded-lg p-2 bg-slate-700 font-medium text-neutral-100 hover:bg-slate-800">Add Customer</button>
                        <button type="button" onClick={() => setShowModalAccountManager(true)} className="inline-flex items-center border rounded-lg p-2 bg-sky-700 font-medium text-neutral-100 hover:bg-sky-800">Add Account Manager</button>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                filteredCustomers.map((customer, index) => (
                                    <tr key={customer.id} onContextMenu={(e) => handleRowClick(e, customer)} onClick={(e) => handleViewCustomer(customer)} className="border-b border-gray-100 hover:bg-blue-50 transition-colors cursor-pointer" title="Click to view/edit customer details">
                                        <td className="py-3 text-gray-400">{index + 1}</td>
                                        <td className="py-3 px-2 font-semibold min-w-xs">{customer.company_name}</td>
                                        {customer.address &&<td className="py-3 px-2 min-w-xs whitespace-normal text-xs hover:scale-140 hover:bg-white/50 transition"> {customer.address}</td> || <td></td>}
                                        <td className="py-3 px-2 truncate">{customer.industry}</td>
                                        <td className="py-3 px-2">{customer.location}</td>
                                        <td className="py-3 px-2">
                                            <div className="font-semibold truncate">{customer.contact_person}</div>
                                            {customer.designation && <div className="text-gray-500 text-xs">{customer.designation}</div>}
                                        </td>
                                        <td className="py-3 px-2">
                                        ...
                                        <td className="py-3 px-2">
                                            {customer.account_manager ? (
                                                <div>
                                                    <div className="font-medium truncate">{customer.account_manager.name}</div>
                                                    <div className="text-xs text-gray-500">{customer.account_manager.position}</div>
                                                </div>
                                            ) : (
                ...
                <ModalCustomer isOpen={showModalCustomer} onClose={handleCustomerModalClose} customer={selectedCustomer} mode={customerModalMode} accountManagers={accountManagers} />
